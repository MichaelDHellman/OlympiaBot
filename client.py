...
class olympiaClient(discord.Client):
    ...
    async def roleUpdateLoop(self):
        ...
        self.fetchSheetData()
        await self.mailMissingParticipants()
        await self.updateRoles()
        await self.messagePresents()
        

    async def mailMissingParticipants(self):
        ...
        await self.mailAbsentees(missing)
    ...
    async def updateRoles(self):
        ...
        with open("sentto.json", 'r') as sent:
            ...
            for i, m in enumerate(present):
                ...
                if self.sheetData[ind][3] == "Yellow":
                    print("adding yellow")
    ...
    async def mailAbsentees(self, absenteeList):
        ...
        with open("sentto.json", 'r') as sent:
            ...
            for i, m in enumerate(present):
                ...
                if self.sheetData[i][3] != "No Preference":
                    try:
                        await m.send(" ")
                        await m.send("Hello, and welcome to ACM Olympics! You have been placed on the " + self.sheetData[ind][3] + ":" + self.sheetData[ind][3].lower() + "_square: team! If this is your first Olympics, thanks for joining us this year, we're going to have a great time!")
                        mailedTo["discord"].append(m.name)
                        events = self.sheetData[ind][4].split(", ")
                        eventsString = ""
                        for e in events:
                            if ("Opening" in e):
                        ...
                        try:
                            dmChannel = await m.create_dm()
                            await dmChannel.send("Hello, and welcome to ACM Olympics! You have been placed on the " + self.sheetData[ind][3] + ":" + self.sheetData[ind][3].lower() + "_square: team! If this is your first Olympics, thanks for joining us this year, we're going to have a great time!")
                            mailedTo["discord"].append(m.name)
                            events = self.sheetData[ind][4].split(", ")
                            eventsString = ""
                            for e in events:
                                ...
                                if ("Closing" in e):
                                    eventsString = eventsString + "Closing Ceremony :pizza: | 5:30 PM to 6:30 PM | Sunday | ECSS 2.201\n\n"
                                    continue
                            await dmChannel.send("As a quick reminder for your own reference, you signed up for the following events: \n\n" + eventsString)
                            await dmChannel.send("We're sure you can't wait for the games to begin, so to tide yourself over, go and visit the #Olympics channel in the Discord, as well as your team channel, and get to know your fellow competitors! If you have any questions, message @Glassshatter on Discord. See you soon!")
                            print("Messaged with create DMChannel")
                        except:
